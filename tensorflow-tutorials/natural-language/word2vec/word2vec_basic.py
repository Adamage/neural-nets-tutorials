...
# Adam Wrobel -> adding checkpoint saver and embeddings-to-tensorboard projector. The rest is copyright Tensorflow
...
if __name__ == "__main__":
  ...
  LOCAL_PATH = os.path.join(ppu.outputs_dir, 'word2vec_from_tf')
  ...
  with graph.as_default():
    ...
    #region ADDING SAVER FOR CHECKPOINTS
  ...
  with tf.Session(graph=graph) as session:
    ...
    # Do if no checkpoints are found.
      ...
      for step in xrange(num_steps):
        ...
        summary, loss_val, _ = session.run([summary_operations, loss, optimizer], feed_dict=feed_dict)
    ...
    saver.save(session, MODEL_PATH, global_step=num_steps)
